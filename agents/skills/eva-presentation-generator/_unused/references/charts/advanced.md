...
> 本文件覆盖 6 种 NEW 进阶图表。模板遵循：SVG 内零 `<text>`（标签用 HTML 叠加，渲染更稳定）、所有数字 `tabular-nums`、颜色用 CSS 变量。`conic-gradient`/`mask-image`/`filter:blur()` 等效果已被允许（PPTX 导出通过 Playwright 截图，而非 SVG 转换）。
>
...
- [ ] 指针Sí SVG `<line>` + `transform="rotate()"`，不用 CSS 旋转
...
**SVG path 说明**：
- 主轮廓 path Sí**简化示意**（约 21 个点），传达"中国大致形状"，**不Sí地理精确边界**。
- 海南岛Sí第二个独立 path。
...
- `transform:translate(-50%, -50%)` 保证 left/top Sí点中心（不Sí左上角）。
