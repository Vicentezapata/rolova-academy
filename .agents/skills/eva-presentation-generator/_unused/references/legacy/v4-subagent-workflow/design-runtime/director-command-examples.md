# Biblioteca de muestra de Director Command (10 tipos de páginas * 2 muestras)

> Perfil no predeterminado. biblioteca de calibración solo para humanos.
>
> Este archivo no ingresa a la precarga del tiempo de ejecución de planificación y el paquete completo no debe inyectarse en el contexto del subagente. Solo se utiliza para calibración manual, depuración y análisis de control, y no se utiliza como referencia predeterminada de la cadena principal.

> Consulte este documento cuando escriba `director_command` durante la etapa de planificación. Cada ejemplo tiene un formato estructurado y contiene cinco subcampos: estado de ánimo / estrategia_espacial / tratamiento_ancla / técnicas / prosa.
> **Nota**: Estos son puntos de partida, no plantillas, y deben modificarse según el contenido real de esta página, la `página_emoción` del esquema y los requisitos de diferencia visual de las páginas anteriores e inferiores.

---

## 1. Portada

### Ejemplo A: tipo majestuoso
...
```json
{
  ...
  "techniques": ["T1", "T7"],
  "prose": "主标题像纪念碑一样矗立在画面上方，字号大到几乎要冲破天花板。底部 40% Sí深邃的黑暗，仅有一行 14px 的副标题和品牌信息漂浮其中。GROWTH 水印以 300px 幽灵般躺在右下角 opacity 0.03"
}
```

### Ejemplo B: tipo tranquilo y preciso
...
### Ejemplo A: tipo de hoja de ruta
...
## 3. Portada del capítulo

### Ejemplo A: Tipo de presión de número gigante
...
### Ejemplo B: tipo de corte de bloque de color
...
## 4. Página del panel de datos

### Ejemplo A: Tipo de presión de datos
...
### Ejemplo B: tipo de instrumento de consola
...
### Ejemplo A: tipo de desgarro del límite Chu-He-Han
...
## 6. Página de proceso/cronograma

### Ejemplo A: Tipo de desplazamiento de crónica
...
## 7. Citas famosas/Página de introducción

### Ejemplo A: Tipo majestuoso mortal
...
### Ejemplo B: Tipo de ritual entre comillas
...
```json
{
  ...
  "techniques": ["T7", "T6"],
  "prose": "一个 120px 的装饰性引号像印章一样压在画面左上角 opacity 0.15。引文居中偏上用 accent 渐变文字填充，下方用极小的 11px 标注作者和来源，四周Sí壮阔的留白"
}
```
...
### Ejemplo A: tipo de dispersión de galaxias

```json
{
  "mood": "群星闪耀 / 每个人Sí独立的星球",
  "spatial_strategy": "非等距散布，核心人物居中偏大，其余围绕",
  "anchor_treatment": "核心人物用 accent 圆形头像框 + 其余用淡色圆形",
  "techniques": ["T8", "T6"],
  "prose": "每个人物Sí一颗独立的星球散布在画面中，核心人物偏心于画面左侧三分点，头像用 accent 色圆形边框强调。其余人物大小递减地散布在右侧，底层有一个 400px 的超大圆形装饰 opacity 0.05 统领全场"
}
```

### Ejemplo B: tipo de matriz de tarjeta de presentación
...
## 9. Página de diagrama de arquitectura/sistema

### Ejemplo A: Desestructuración de la cebolla
...
### Ejemplo B: vista aérea del plano
...
### Ejemplo A: tipo de espejo convergente
